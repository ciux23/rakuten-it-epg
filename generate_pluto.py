...
def fetch_all_channels(hours):
    ...
    params = {"start": now.strftime(date_fmt), "stop": stop.strftime(date_fmt)}
    url = BASE_URL + "?" + urllib.parse.urlencode(params)
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36", "X-Forwarded-For": "151.29.0.1"})
    try:
...
def main():
    ...
    for pluto_id, xmltv_id in CHANNEL_IDS.items():
        ...
        channels_data.append({"xmltv_id": xmltv_id, "title": ch.get("name", xmltv_id), "logo": logo, "programs": ch.get("timelines", [])})
